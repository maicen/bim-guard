...
- **Navigation**: Path-based via `svelte-spa-router` (hash routing, e.g. `/#/rules`) — `App.svelte` derives the active view from `router.location` and navigates with `push()`/`replace()`. Project/element/BCF targeting is carried as a `?project_id=...` query string read reactively off `router.querystring`, not as one-shot props. Do not reintroduce a local `activeView` state variable.
...
- **IFC 3D Viewer**: `src/lib/components/IfcViewer.svelte` is a hybrid. The 3D engine — `Components`/`Worlds`/`FragmentsManager`/`IfcLoader`, and every ThatOpen class it drives (`Views`, `ItemsFinder`, `Hider`, `TechnicalDrawings`, `DxfManager`, `BCFTopics`/`Viewpoints`) — lives entirely in `static/js/viewer/ifc-viewer.js`, loaded at runtime via pinned esm.sh CDN imports and dynamically `import()`ed, deliberately isolated from the Vite bundle (worker/wasm asset loading and guaranteeing a single `three.js` instance are real risks not worth taking on for this). That module exposes a typed bridge object from `initViewer()` — imperative methods plus `onChange`/`onSelectionChange` subscriptions (see `isolate.onSelectionChange` for the original pattern) — and hands over a few raw DOM mount points for widgets it re-exports (the `bim-viewport`, the BCF topic-detail CUI panel, the `bim-sheet-board` drawing sheet). **All ribbon/panel/button chrome around it is real Svelte** — `ViewerRibbon.svelte`, `LayersPanel.svelte`, `DrawingsPanel.svelte` under `src/lib/components/viewer/`, `CollapsiblePanel.svelte` for docking — driven entirely by that bridge. Never hand-roll more `BUI.Component.create()` DOM for new viewer UI; add a bridge method + subscription and build the control in Svelte instead.
