...
from fasthtml.common import A, Details, Div, Form, H3, Option, P, Pre, Request, Span, Summary, Table, Tbody, Td, Th, Thead, Title, Tr
...
def _rule_compliance_card(compliance_results: list[dict], summary: dict, error: str | None):
    ...
    rate_cls = (
        "bg-green-100 text-green-800" if rate >= 80
        else "bg-yellow-100 text-yellow-800" if rate >= 50
        else "bg-red-100 text-red-800"
    )

    summary_bar = Div(
        Span(f"{rate:.0f}% pass rate",
        ...
        Span(f"  ⚠ {mand_f} mandatory failures",
             cls="text-xs text-red-600 font-semibold ml-2") if mand_f else "",
    ...
    for r in compliance_results:
        ...
        if failures:
            ...
            fail_detail = Details(
                Summary(f"{len(failures)} failing element(s)",
                        cls="text-xs text-red-600 cursor-pointer"),
                Div(
                    Table(
                        Thead(Tr(
        ...
        rows.append(Tr(
            ...
            Td(Span(status, cls=f"inline-block px-1.5 py-0.5 rounded text-xs font-semibold {s_cls}"),
               cls="px-3 py-2"),
    ...
    return Card(
        CardHeader(
            Div(
                CardTitle(title),
                P("Full property-level compliance check against every rule in the library.",
                  cls="text-xs text-muted-foreground mt-0.5"),
            )
        ),
        CardContent(
