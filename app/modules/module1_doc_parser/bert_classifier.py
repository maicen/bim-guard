...
class BERTClassifier:
    ...
    def _load_codeaccord_data(self) -> tuple:
        ...
        try:
            ...
            text_field  = next((f for f in ("sentence", "Sentence", "text", "Text") if f in sample), None)
            label_field = next((f for f in ("is_rule", "IsRule", "label", "Label", "compliance") if f in sample), None)
            ...
            def format_row(row):
                ...
                return {
                    ...
                    "label": 1 if (raw_label is True or raw_label == 1 or str(raw_label).lower() in ("true", "1", "yes", "rule")) else 0,
                }

            train = [format_row(r) for r in dataset["train"]]
            eval_ = [format_row(r) for r in dataset.get("validation", dataset.get("test", dataset["train"][-50:]))]
