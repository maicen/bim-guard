...
def _viewpoint_xml(issue: BCFIssue, viewpoint_guid: str) -> str:
    ...
    ``Component/@IfcGuid`` is only written when ``issue.component_guid`` is a
    real 22-character IFC GlobalId (see :func:`_ifc_guid_attr`).
    """
    component_guid_attr = _ifc_guid_attr(issue.component_guid)
    return f"""<?xml version="1.0" encoding="UTF-8"?>
<VisualizationInfo Guid="{_xml_attr(viewpoint_guid)}">
  <Components>
    <Selection>
      <Component{component_guid_attr}>
