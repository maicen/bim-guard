...
<div class="mx-auto space-y-6">
  <!-- Header -->
  <div>
    <div class="mb-1 text-xs font-bold uppercase tracking-widest text-fg-muted">Manuals</div>
  ...
  <div class="space-y-4 rounded-2xl border border-border-default bg-surface-card/40 p-6">
    ...
    {#each CHECKLISTS as group (group)}
      <div class="space-y-2">
        <div class="text-xs font-semibold text-fg-primary">{group.domain}</div>
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
          {#each group.items as item (item)}
            <div class="flex gap-3 rounded-xl border border-border-default bg-surface-canvas/60 p-4">
              <CheckCircle2 class="mt-0.5 h-4 w-4 shrink-0 text-accent" />
              <div class="space-y-1">
                <div class="text-xs font-semibold text-fg-primary">{item.title}</div>
                <p class="text-caption leading-relaxed text-fg-muted">{item.why}</p>
              </div>
            </div>
          {/each}
        </div>
        ...
        <div class="grid grid-cols-1 gap-3 md:grid-cols-3">
          {#each group.items as item (item)}
            <div class="space-y-2 rounded-xl border border-border-default bg-surface-canvas/60 p-4">
              <div class="text-xs font-semibold text-fg-primary">{item.element}</div>
              <p class="text-caption leading-relaxed text-fg-muted">
                {item.rules}
              </p>
            </div>
          {/each}
