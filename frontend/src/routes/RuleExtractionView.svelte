...
<div class="space-y-6 max-w-6xl mx-auto">
  <!-- Header -->
  <div>
    <div class="text-xs font-bold uppercase tracking-widest text-slate-400 mb-1">Studio</div>
    <h1 class="text-2xl sm:text-3xl font-bold tracking-tight text-white">Rule Extraction Studio</h1>
    <p class="text-xs sm:text-sm text-slate-400">
      Transform building codes, engineering standards, and specification documents into structured OpenBIM verification rules.
    </p>
  </div>
  ...
  <!-- Configuration Box -->
  <div class="p-6 rounded-2xl bg-slate-900/60 border border-slate-800 space-y-5">
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <!-- Source Document Selection -->
      <div>
        <label for="extract-doc" class="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-1.5">
        ...
        >
          ...
          {#each documents as doc}
            <option value={doc.id}>{doc.filename} ({doc.char_count.toLocaleString()} chars)</option>
          {/each}
        </select>
      </div>
      ...
      <div>
        <label for="extract-model" class="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-1.5">
    ...
    {#if !selectedDocId}
      <div>
        <label for="extract-text" class="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-1.5">
    ...
    <div class="space-y-4">
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-xs text-slate-400">Review, modify properties, and select rules to persist to the library.</p>
        </div>
                ...
                <th class="py-3 px-3 w-10 text-center">
                  <input
                    ...
                    on:change={(e) => {
                      ...
                      extractedRules = extractedRules.map((r) => ({ ...r, selected: chk }));
                    }}
            ...
            <tbody class="divide-y divide-slate-800/60">
              {#each extractedRules as rule, i}
                <tr class="hover:bg-slate-900/60 transition-colors">
                  ...
                  <td class="py-3 px-3 text-slate-400 font-mono">{rule.operator || '=='}</td>
                  <td class="py-3 px-3 font-mono text-cyan-300">
                    {rule.check_value || (rule.value_min ? `[${rule.value_min}..${rule.value_max}]` : '-')}
                  </td>
                  <td class="py-3 px-3">
                    <select
