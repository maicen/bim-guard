    ...
    <div class="bg-slate-900 border border-slate-800 w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
      ...
      <div class="px-6 py-4 border-b border-slate-800 flex items-center justify-between">
        <div class="flex items-center gap-2.5">
          ...
          <div>
            <h2 class="text-base font-bold text-slate-50 tracking-tight">{project.name}</h2>
      ...
      <div class="p-6 space-y-4 overflow-y-auto text-xs">
        ...
        <div class="grid grid-cols-2 gap-3">
          <div class="p-3 bg-slate-950/40 rounded-xl border border-slate-800">
            <span class="text-slate-500 block text-micro uppercase tracking-wider font-semibold mb-1">Status</span>
            <span
              class="inline-block px-2.5 py-0.5 rounded-full text-micro font-semibold {project.status === 'Active'
          ...
          <div class="p-3 bg-slate-950/40 rounded-xl border border-slate-800">
            <span class="text-slate-500 block text-micro uppercase tracking-wider font-semibold mb-1">Jurisdiction</span>
          ...
          <div class="p-3 bg-slate-950/40 rounded-xl border border-slate-800">
            <span class="text-slate-500 block text-micro uppercase tracking-wider font-semibold mb-1">Analysis Domain</span>
            <div class="flex items-center gap-1.5 text-slate-300 font-medium">
          ...
          <div class="p-3 bg-slate-950/40 rounded-xl border border-slate-800">
            <span class="text-slate-500 block text-micro uppercase tracking-wider font-semibold mb-1">Created At</span>
        ...
        <div class="p-4 rounded-xl bg-slate-950 border border-slate-800 space-y-2">
          <span class="text-slate-400 text-micro uppercase tracking-wider font-semibold block">ISO 19650 Container Naming & CDE Governance</span>
          <div class="grid grid-cols-3 gap-2">
            <div class="p-2 rounded-lg bg-slate-900 border border-slate-800">
              <span class="text-nano text-slate-500 block font-semibold">Suitability</span>
              <span class="text-xs font-bold text-amber-400">{project.suitability_code || 'S0'}</span>
            </div>
            <div class="p-2 rounded-lg bg-slate-900 border border-slate-800">
              <span class="text-nano text-slate-500 block font-semibold">Revision</span>
              <span class="text-xs font-bold text-blue-400">{project.revision_code || 'P01.01'}</span>
            </div>
            <div class="p-2 rounded-lg bg-slate-900 border border-slate-800">
              <span class="text-nano text-slate-500 block font-semibold">CDE State</span>
              <span class="text-xs font-bold text-emerald-400">{project.cde_state || 'WIP'}</span>
            </div>
          </div>
          {#if project.project_code || project.originator}
            <div class="text-caption font-mono text-slate-300 pt-1">
              Container Tag: <span class="text-slate-50 font-semibold">[{project.project_code || 'PRJ'}]-[{project.originator || 'ORIG'}]-[{project.volume_system || 'ZZ'}]-[{project.level || 'ZZ'}]-[{project.type || 'M3'}]-[{project.role || 'A'}]-[{project.number || '0001'}]</span>
            </div>
          {/if}
        </div>
        ...
        <div class="p-4 rounded-xl bg-slate-950 border border-slate-800 space-y-3">
          <div class="flex items-center justify-between">
            <span class="text-slate-400 text-micro uppercase tracking-wider font-semibold block">Attached OpenBIM Model</span>
            {#if project.ifc_file_path}
              <span class="inline-flex items-center gap-1 text-micro font-semibold text-emerald-400 bg-emerald-950/60 border border-emerald-800/60 px-2 py-0.5 rounded-full">
          ...
          {#if project.ifc_file_path}
            <div class="flex items-center justify-between">
              ...
              <div class="flex items-center gap-2">
                <a
                  ...
                  download
                  class="inline-flex items-center gap-1 px-3 py-1 rounded-lg text-xs bg-slate-800 hover:bg-slate-700 text-slate-50 transition-colors"
                >
            ...
            {#if project.ifc_md5_hash}
              <div class="text-caption font-mono text-slate-500 truncate pt-1">
