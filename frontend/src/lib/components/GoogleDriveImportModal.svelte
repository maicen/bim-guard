...
<Modal isOpen={true} title="Import from Google Drive" icon={CloudDownload} maxWidth="max-w-xl" {onClose}>
  <p class="text-fg-muted">
    ...
    <span class="font-semibold text-fg-secondary">"Anyone with the link"</span> — a private file will fail
    with a clear error below rather than aborting the whole batch.
  </p>

  <div class="space-y-1.5">
    <label for="drive-urls" class="text-xs font-semibold text-fg-secondary">Drive links or file IDs</label>
    <textarea
      id="drive-urls"
      bind:value={urlsText}
      rows="5"
      placeholder="https://drive.google.com/file/d/.../view?usp=sharing"
      class="w-full rounded-lg border border-border-interactive bg-surface-canvas px-3 py-2 font-mono text-xs text-fg-primary placeholder:text-fg-muted focus:border-accent focus:outline-hidden"
    ></textarea>
  </div>
  ...
  {#if results}
    <div class="space-y-1.5 rounded-lg border border-border-default bg-surface-canvas/60 p-3">
      {#each results as result (result.url)}
        <div class="flex items-start gap-2 text-xs">
          ...
          {/if}
          <div class="min-w-0">
            <p class="truncate text-fg-secondary">{result.document?.filename || result.url}</p>
            {#if !result.ok}
