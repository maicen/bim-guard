...
>
  ...
  <div class="relative flex-1 min-w-[14rem] max-w-md">
    ...
    <input
      ...
      class="w-full bg-slate-950 border border-slate-800 rounded-xl pl-9 pr-8 py-2 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-blue-500 transition-colors"
    />
    {#if searchQuery}
      <button
        type="button"
  ...
  <div class="flex items-center gap-2.5 flex-wrap">
    ...
    {#if onRefresh}
      <button
        ...
        class="p-2 rounded-xl border border-slate-800 bg-slate-950 hover:bg-slate-800 text-slate-400 hover:text-white disabled:opacity-50 transition-colors"
        title="Refresh data"
      >
