...
<div class="rounded-2xl border border-border-default bg-surface-card/40">
  <button
    ...
    onclick={togglePanel}
    aria-expanded={open}
    class="flex w-full items-center gap-2 rounded-2xl px-4 py-3 text-left transition-colors hover:bg-surface-hover"
  >
    <BookOpen class="h-4 w-4 shrink-0 text-amber-400" />
    <span class="text-sm font-bold text-fg-primary">What do these checks do?</span>
    <ChevronDown
      class="ml-auto h-4 w-4 shrink-0 text-fg-muted transition-transform {open
        ? 'rotate-180'
        : ''}"
    />
  </button>
  ...
  {#if open}
    <div class="space-y-5 border-t border-border-default px-4 py-4">
      ...
      <section class="space-y-2">
        ...
        <ul class="ml-4 list-disc space-y-1 text-xs text-fg-secondary sm:text-sm">
          <li><strong class="text-fg-primary">Low</strong> — nothing to worry about.</li>
          <li><strong class="text-fg-primary">Medium</strong> — worth a designer's attention.</li>
          <li>
            <strong class="text-fg-primary">High</strong> — likely to cause a problem; change something.
          </li>
          <li>
            <strong class="text-fg-primary">Critical</strong> — this will fail; do not build it like this.
          </li>
          <li>
            <strong class="text-fg-primary">Undetermined</strong> — the model didn't contain enough information
      ...
      {#snippet row(id: string, title: string, body: Snippet)}
        ...
        <div
          class="overflow-hidden rounded-xl border transition-colors {on
            ...
            : 'border-border-default bg-surface-card/60'}"
        >
          <button
            type="button"
          ...
          >
            <span
              class="font-mono text-caption font-semibold {on ? 'text-amber-300' : 'text-fg-muted'}"
            >
              {title}
            </span>
            {#if !on}
              <span class="font-mono text-micro uppercase tracking-wider text-fg-muted"
                >not selected</span
              >
            {/if}
            <ChevronDown
              class="ml-auto h-3.5 w-3.5 shrink-0 {on
                ? 'text-amber-400'
                : 'text-fg-muted'} transition-transform {expanded === id ? 'rotate-180' : ''}"
            />
          </button>
          {#if expanded === id}
            <div
              class="space-y-2 border-t border-border-default px-3 py-3 text-xs text-fg-secondary sm:text-sm"
            >
              {@render body()}
            </div>
          {/if}
      ...
      {#snippet gc()}
        <p>
          <strong class="text-fg-primary">The everyday version.</strong> If you connect two different metals
        ...
        <p>
          <strong class="text-fg-primary">What the check asks.</strong> For each pipe: what metal is it, what
        ...
        <p>
          <strong class="text-fg-primary">What it needs from the model.</strong> The pipe's material, its partner's
        ...
        <p>
          <strong class="text-fg-primary">Example.</strong> A copper pipe with a galvanised-steel support bracket
          in a plant room: <strong class="text-fg-primary">Medium</strong>. A stainless pipe on a stainless bracket:
          <strong class="text-fg-primary">Low</strong>. A pipe with no material recorded:
          <strong class="text-fg-primary">Undetermined — material not resolved</strong>.
        </p>
      {/snippet}

      {#snippet cc()}
        <p>
          <strong class="text-fg-primary">The everyday version.</strong> Think of a tiny gap — under a washer,
      ...
      {#snippet mc()}
        <p>
          <strong class="text-fg-primary">The everyday version.</strong> Bacteria live in water pipes. Mostly they're
        ...
        <p>
          <strong class="text-fg-primary">What the check asks.</strong> Three things about the water in each pipe: how
        ...
        <p>
          <strong class="text-fg-primary">What it needs from the model.</strong> Flow speed, water temperature, and
        ...
        <p>
          <strong class="text-fg-primary">Example.</strong> A condensate drain at 30 °C with no flow:
          <strong class="text-fg-primary">Critical</strong>. The same pipe carrying water at 65 °C:
          <strong class="text-fg-primary">Low</strong> — too hot for the bacteria. A pipe with no flow or temperature
          data: <strong class="text-fg-primary">Undetermined — hydraulic data unavailable</strong>.
        </p>
      {/snippet}

      {#snippet mm()}
        <p>
          <strong class="text-fg-primary">The everyday version.</strong> Some materials just shouldn't carry some
        ...
        <p>
          <strong class="text-fg-primary">What the check asks.</strong> For each pipe: what's it made of, and what
        ...
        <p>
          <strong class="text-fg-primary">What it needs from the model.</strong> The material and the system the pipe
        ...
        <p>
          <strong class="text-fg-primary">Example.</strong> Galvanised steel on a hot-water system:
          <strong class="text-fg-primary">Medium</strong> (zinc coating will fail). Copper on cold water:
          <strong class="text-fg-primary">Low</strong>. A material the table doesn't cover:
          <strong class="text-fg-primary">Undetermined — pairing not mapped</strong>.
        </p>
      {/snippet}

      {#snippet xm()}
        <p>
          <strong class="text-fg-primary">The everyday version.</strong> Walk along a pipe run and you'll find places
        ...
        <p>
          <strong class="text-fg-primary">What the check asks.</strong> Where does one material connect directly to a
        ...
        <p>
          <strong class="text-fg-primary">What it needs from the model.</strong> Materials, and how the pipes connect to
        ...
        <p>
          <strong class="text-fg-primary">Example.</strong> A copper-to-galvanised-steel joint on a wet system:
          <strong class="text-fg-primary">High</strong>. A copper-to-brass joint:
          <strong class="text-fg-primary">Medium</strong> or <strong class="text-fg-primary">Low</strong> depending on the
      ...
      <section class="space-y-2 border-t border-border-default pt-4">
        <h3 class="text-xs font-bold uppercase tracking-wider text-fg-muted">
          Why five checks and not one
        </h3>
        <p class="text-xs text-fg-secondary sm:text-sm">
          ...
          <strong class="text-fg-primary">Low</strong> — no second metal, no battery — while the crevice check
          gives them <strong class="text-fg-primary">Critical</strong>. If you only ran one check, you'd sign
        ...
        <ul class="ml-4 list-disc space-y-1 text-xs text-fg-secondary sm:text-sm">
          <li>
            <strong class="text-fg-primary">Checks 1, 2 and 3 look at one pipe at a time.</strong> They need to
          ...
          <li>
            <strong class="text-fg-primary">Checks 4 and 5 look at the whole network.</strong> They need to know
