...
<div class="space-y-4">
  ...
  {#if catalogError}
    <div class="p-3 rounded-xl border border-amber-500/30 bg-amber-500/5 text-caption text-amber-300">
  ...
  <div class="flex flex-wrap gap-1 border-b border-slate-800 pb-2">
    {#each TABS as tab}
      <button
        ...
        class="px-3 py-1.5 rounded-lg text-caption font-semibold transition-colors {activeTab === tab.id
  ...
  {#if activeTab === 'metadata'}
    <div class="grid grid-cols-2 gap-3">
      <div>
        <label for="naming-project-code" class="block text-caption font-medium text-slate-400 mb-1.5">
          Project Code
        </label>
        <input
          ...
          placeholder="A1234"
          class="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-sm text-slate-50 font-mono focus:outline-none focus:border-accent"
        />
      </div>
      <div>
        <label for="naming-originator" class="block text-caption font-medium text-slate-400 mb-1.5">
          Originator Code
        </label>
        <input
      ...
      <div>
        <label for="naming-type" class="block text-caption font-medium text-slate-400 mb-1.5">
          Information Type
        </label>
        <select
          id="naming-type"
      ...
      <div>
        <label for="naming-suitability" class="block text-caption font-medium text-slate-400 mb-1.5">
      ...
      <div>
        <label for="naming-revision" class="block text-caption font-medium text-slate-400 mb-1.5">
          Revision
        </label>
        <input
          id="naming-revision"
          ...
          class="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-sm text-slate-50 font-mono focus:outline-none focus:border-accent"
        />
        <p class="text-micro text-slate-500 mt-1">P01 preliminary · C01 contract</p>
      </div>
      <div>
        <label for="naming-date-format" class="block text-caption font-medium text-slate-400 mb-1.5">
          Date Format
        </label>
        <select
      ...
      <div>
        <label for="naming-separator" class="block text-caption font-medium text-slate-400 mb-1.5">
          Separator
        </label>
        <select
          id="naming-separator"
    ...
    {#if config.active_convention === 'uniclass'}
      ...
      <div class="grid grid-cols-2 gap-3 pt-3 border-t border-slate-800">
        <div>
          <label for="naming-class-a" class="block text-caption font-medium text-slate-400 mb-1.5">
            Uniclass Primary
          </label>
          <input
            id="naming-class-a"
        ...
        <div>
          <label for="naming-class-b" class="block text-caption font-medium text-slate-400 mb-1.5">
            Uniclass Secondary
          </label>
          <input
            id="naming-class-b"
  ...
  {:else if codeTab}
    <div class="space-y-3">
      <div>
        <span class="block text-caption font-medium text-slate-400 mb-1.5">
          Active {codeTab.noun} codes
        </span>
        {#if selected.length === 0}
          <p class="text-caption text-slate-500 p-3 rounded-xl border border-slate-800">
            None selected — the whole library is available to this project.
          </p>
        {:else}
          <div class="flex flex-wrap gap-1.5">
            {#each selected as code}
              <span class="inline-flex items-center gap-1.5 pl-2.5 pr-1.5 py-1 rounded-lg bg-slate-800 text-caption text-slate-50">
                ...
                <button
                  ...
                  class="p-0.5 rounded hover:bg-slate-700 text-slate-400 hover:text-slate-50"
                >
                  <X class="w-3 h-3" />
                </button>
              </span>
          ...
          <div class="flex flex-wrap gap-1.5">
            {#each available as code}
              <button
                ...
                class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg border border-slate-800 hover:border-accent text-caption text-slate-300 hover:text-slate-50 transition-colors"
              >
                <Plus class="w-3 h-3" />
                <span class="font-mono font-semibold">{code.code}</span>
      ...
      <div class="pt-3 border-t border-slate-800 flex items-end gap-2">
        <div class="w-24">
          <label for="naming-custom-code" class="block text-caption font-medium text-slate-400 mb-1.5">
            Custom code
          </label>
          <input
        ...
        <div class="flex-1">
          <label for="naming-custom-label" class="block text-caption font-medium text-slate-400 mb-1.5">
            Meaning
          </label>
          <input
        ...
        <button
          ...
          class="px-4 py-2 rounded-xl text-caption font-semibold bg-slate-800 hover:bg-slate-700 disabled:opacity-40 text-slate-50 transition-colors"
        >
          Add
        </button>
      </div>
      ...
      <div class="rounded-xl border border-slate-800 divide-y divide-slate-800">
        {#each catalog?.cde_statuses ?? [] as s}
          <div class="flex items-center gap-3 px-3 py-2">
            ...
            <span class="font-mono text-xs font-semibold text-slate-50 w-8">{s.code}</span>
            <span class="text-caption text-slate-300 flex-1">{s.label}</span>
            {#if !s.selectable}
              <span class="text-micro text-slate-500 uppercase tracking-wider">Reference</span>
            {/if}
          </div>
        {/each}
      </div>
        ...
        >
          <div class="flex items-center gap-2">
            ...
            {#if !convention.iso_compliant}
              <span class="text-micro px-1.5 py-0.5 rounded bg-amber-500/15 text-amber-400">
                Not ISO compliant
              </span>
            {/if}
            {#if !convention.preset}
              <span class="text-micro px-1.5 py-0.5 rounded bg-slate-800 text-slate-400">Custom</span>
            {/if}
          </div>
          <p class="text-micro text-slate-500 mt-1">{convention.description}</p>
          <p class="text-micro font-mono text-slate-400 mt-1.5 break-all">{convention.format}</p>
        </button>
      {/each}
    </div>
  {/if}
  ...
  <div class="p-3 rounded-xl border border-slate-800 bg-slate-950/60">
    <div class="flex items-center gap-1.5 mb-1.5">
      ...
      <span class="text-micro font-semibold text-slate-400 uppercase tracking-wider">
    ...
    {#if preview}
      ...
      {#if appliedFormat}
        <p class="font-mono text-micro text-slate-500 mt-1 break-all">{appliedFormat}</p>
      {/if}
      {#if unresolved.length}
        <p class="text-micro text-amber-400/80 mt-1">
