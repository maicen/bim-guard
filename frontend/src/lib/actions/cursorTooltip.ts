...
export function cursorTooltip(node: Element, options: CursorTooltipOptions = {}) {
  ...
  function show(e: PointerEvent) {
    ...
    if (!tooltipEl) {
      ...
      tooltipEl.className =
        "pointer-events-none fixed z-70 max-w-64 whitespace-pre-line rounded-md border border-border-default bg-surface-card/95 px-2 py-1 text-micro text-fg-secondary shadow-lg shadow-black/30 backdrop-blur-xs";
