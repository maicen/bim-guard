...
export function cursorTooltip(node: HTMLElement, options: CursorTooltipOptions = {}) {
  ...
  function show(e: PointerEvent) {
    ...
    if (!tooltipEl) {
      ...
      tooltipEl.className =
        "pointer-events-none fixed z-[70] max-w-64 rounded-md border border-slate-800 bg-slate-900/95 px-2 py-1 text-micro text-slate-200 shadow-lg shadow-black/30 backdrop-blur-sm";
