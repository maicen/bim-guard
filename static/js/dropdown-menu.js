(() => {
    if (window.__bimguardDropdownInit) return;
    window.__bimguardDropdownInit = true;

    const ROOT_SELECTOR = '[data-dropdown-menu="root"]';
    const SUB_SELECTOR = '[data-dropdown-menu="sub"]';

    const closeCustomDropdownsOnOutsideClick = (event) => {
        const clickInsideAnyMenu = !!event.target.closest(ROOT_SELECTOR);
        if (clickInsideAnyMenu) return;

        document.querySelectorAll(ROOT_SELECTOR).forEach((menu) => {
            if (menu.open) {
                menu.open = false;
            }

            menu.querySelectorAll(SUB_SELECTOR).forEach((sub) => {
                sub.open = false;
            });
        });
    };
    ...
    document.addEventListener('click', (event) => {
        closeCustomDropdownsOnOutsideClick(event);
    });
