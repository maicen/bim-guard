...
- Of those, only **2,403** (2.07%) normalise to a `CANONICAL_MATERIALS` key the engines can score — see §A.1.1
...
## Table A.2 — Severity distribution
...
## Table A.3 — Material x corrosion engine
...
## Table A.4 — Engine band distribution and input coverage
...
## Table A.6 — Standards sensitivity
...
## Table A.7 — IFC schema comparison
...
## Table A.7b — Schema twins (controlled)
...
- **The clearance sensitivity sweep (Table A.6) covers rows [5, 9, 12, 14, 15] only**,
