...
> Wall-clock cost of each pipeline stage against element count, generating Halo clearance volumes at LOD 300 with a 500 mm buffer from the reference IFC4 model (9152ac527a1844f69a73f73e77468326_BUILDING_R4.ifc). Markers are medians of n = 7 complete runs and error bars span the inter-quartile range (Q1–Q3); the vertical axis is logarithmic. IFC triangulation exceeds Halo generation by roughly two orders of magnitude at every count — 30.1 s against 0.39 s at 1,000 elements. Measured on a 4-core x86_64 Linux VM with 15.7 GB RAM, Python 3.11.15, IfcOpenShell 0.8.5, NumPy 2.4.6.

**Figure 5.2** — `fig2_throughput.png`

> Halo generation throughput, derived from the median generation time of n = 7 runs per scenario, at LOD 300 with a 500 mm buffer. Throughput is effectively flat across the range, because generation cost is constant per element and independent of the source element's polygon count. Measured on a 4-core x86_64 Linux VM with 15.7 GB RAM, Python 3.11.15, IfcOpenShell 0.8.5, NumPy 2.4.6.
