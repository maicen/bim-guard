...
│              → RuleService.create_rule() → rules table (Supabase/SQLite)   │
│ CLI prototype :                              │
...
**Seed rules.** `obc_seed_rules.py` contains 31 pre-built OBC Part 9 rules, and `obc_extended_rules.py` adds 14 more OBC Part 9/Part 3 rules, both seeded automatically at application startup — **45 pre-built rules** in total, each with a real OBC section reference. These seed rules are consumed identically by the generic comparator in §1.2.3, regardless of whether they were seeded, LLM-extracted, or regex-extracted — a direct benefit of keeping extraction and evaluation separate.
...
- **Iteration 0, Streamlit prototype.** The team's first working end-to-end system was a Streamlit application. It is not present in this repository's git history, but its influence is: the corrosion module's own `issue_tracker.py` file still carries a `"Usage in Streamlit: from modules.issue_tracker import IssueTracker"` docstring and an embedded Streamlit integration snippet, and it produced the persisted `data/bimguard_issue_history.json` file discussed in §1.4.2 — real historical data whose generating code was left behind in the migration.
- **Iteration 1, Mock-first pipeline.** The rule-extraction path was first built with `module3_rule_builder_mock.py` and the OBC seed rules, letting Components 2 and 3 be developed end-to-end before the NLP layer was reliable. This mock module is now orphaned dead code.
...
- **Iteration 9, discipline-based rule sorting.** As the rule library grew to include both OBC dimensional rules and corrosion mechanisms, `RuleService` gained a `THEMES`/`infer_theme()` classifier so a compliance run can be scoped to Architecture or MEP rules specifically, rather than always evaluating the full combined rule set (§1.2.3).
...
- **Cross-jurisdiction variability.** Mitigation: rulesets are stored as JSON with named standards in `data/rulesets/`, so jurisdictions can in principle be added without touching engine logic — though today these JSON files are consumed only for documentation/Rule Library display, not as the live parametrisation of the corrosion engine (§1.2.4), so this mitigation is partially realised.
...
**Tier (b), corrected from an earlier draft of this report.** A persisted run-history file, `data/bimguard_issue_history.json`, was inspected directly. It contains 49 tracked elements (`IfcPipeSegment`, `IfcPipeFitting`, `IfcFastener`, `IfcHeatExchanger`, `IfcDuctSegment`, `IfcValve`, and others) with per-element event logs (`raised`/`resolved`, each carrying a real `composite_score` and `risk_band`), timestamped **2026-04-09, 18:44–18:47 UTC**. Filtering to currently-open (unresolved) issues gives exactly:
...
*Table 2. Currently-open corrosion issues, computed directly from `data/bimguard_issue_history.json`.*
...
2. Four independent implementations of galvanic/crevice-style scoring exist across the codebase, with only `compliance_runner.py` live; consolidation is needed, ideally parametrised directly from `data/rulesets/*.json`.
...
10. Rule extraction is validated only on OBC dimensional clauses, not the full breadth of codes; the "Halo" volumetric-clearance capability remains conceptual; cost/schedule figures rely on hardcoded default UK rates rather than a checked-in, editable CSV.
...
4. Wire the MIC engine into `compliance_runner.py`, ideally reading tables from `data/rulesets/*.json` instead of hardcoded Python tables.
...
- **Appendix D — Evaluation harness, tests, and historical run data.** `app/modules/tests/` (57 test functions: `test_module1.py`, `test_module3.py`, `test_compliance.py`, `test_integration.py`, `conftest.py`) and `eval_harness.py` (golden `EVAL_CASES`, LLM-as-judge scoring) — see §1.2.8 and §1.4.5 for fixes needed. `data/bimguard_issue_history.json` — persisted corrosion-engine run history from the Streamlit-era predecessor system, the evidentiary basis for §1.4.2.
