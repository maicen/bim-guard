...
- **Strict Prohibition of Raw HTML Controls (MANDATORY)**: Never use raw browser controls (`<select>`, `<input type="checkbox">`, `<input type="radio">`, `<input type="range">`, `<input type="date">`, `<details>`, or unstyled `title="..."` attributes). All interactive controls across components, views, dialogs, and viewers MUST use the corresponding `bits-ui` primitives from `frontend/src/lib/components/ui/` and `frontend/src/lib/components/`.
- **bits-ui Component Suite**: Target all interactive, disclosure, floating, navigation, and input patterns to the 41 headless `bits-ui` primitives in `frontend/src/lib/components/ui/` (`Dialog`, `AlertDialog`, `Popover`, `Tooltip`, `ContextMenu`, `DropdownMenu`, `Command`, `Menubar`, `NavigationMenu`, `Tabs`, `Pagination`, `Button`, `Input`, `FormField`, `Checkbox`, `RadioGroup`, `Switch`, `Select`, `Combobox`, `Slider`, `Label`, `PinInput`, `DatePicker`, `DateRangePicker`, `Calendar`, `RangeCalendar`, `TimeField`, `TimeRangeField`, `Accordion`, `Collapsible`, `ScrollArea`, `Separator`, `AspectRatio`, `Progress`, `Meter`, `Avatar`, `RatingGroup`, `LinkPreview`, `Toolbar`, `Toggle`, `ToggleGroup`). Never hand-roll focus containment, backdrop dismissing, floating coordinates, or keyboard navigation.
